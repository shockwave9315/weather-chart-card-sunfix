...
This repository is a **derivative build** of the original project at:
- **Upstream:** https://github.com/mlamberts78/weather-chart-card

All copyrights remain with the original authors. This fork is published under the **same license** as upstream.
**Not authored by me** — only minimal changes were applied:

### Changes in this build
- Adds two optional config options for per-location sunrise/sunset:
  - `sunrise_entity` (sensor with `device_class: timestamp`)
  - `sunset_entity`  (sensor with `device_class: timestamp`)
- The card prefers these over `sun.sun` (fallback still supported).
- Guards editor against missing `config.forecast`.
- Avoids double registration errors with `customElements.get(...)` guards.

### Example Lovelace config
```yaml
type: custom:weather-chart-card
entity: weather.open_meteo_2
show_sun: true
sunrise_entity: sensor.open_meteo_wschod_slonca_2
sunset_entity: sensor.open_meteo_zachod_slonca_2
